...
object PlanItemForm {
    ...
    fun validate(draft: PlanItemDraft, snapshot: FinanceSnapshot): Result {
        ...
        if (draft.type == FlowType.TRANSFER && snapshot.isAutoManagedDebt(draft.toAccountId) && !draft.extraRepayment) {
            // 卡片選了「照計畫編的金額」時，這個項目就是那張卡每期要繳的錢（R-CARD-27），不是被忽略。
            warnings += if (snapshot.paysCardFromPlan(draft.toAccountId)) {
                val name = snapshot.account(draft.toAccountId)?.name ?: "這張卡"
                "這些金額就是「$name」每期要繳的錢；沒繳完的部分會照利率計息"
            } else {
                "轉入的帳戶已依合約自動繳款，這個項目不會計入；若是額外還款請勾選「額外還款」"
            }
        }
